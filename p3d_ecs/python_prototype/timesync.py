...
class TimeSynchronization(object):
    ...
    def time_information_sufficient(self):
        return len(self._confirmed_syncs) >= self.MIN_CONFIRMED_SYNCS
    ...
    def handle_time_sync_response(self, vclient, data):
        ...
        if travel_time > self.MAX_TRAVEL_TIME:
            self._client.log("Got time sync, but outside of max travel time (max=",
                             self.MAX_TRAVEL_TIME, "actual=", travel_time, ")")
            return
    ...
    def _average_syncs(self):
        """ Averages the last n syncs to find out the simulation time """
        if self.time_information_sufficient:
            best_results = sorted(self._confirmed_syncs, key=lambda v: v[1])
            avg_deltas = []
            for delta, travel_time in best_results[0:self.MIN_CONFIRMED_SYNCS // 2]:
                avg_deltas.append(delta)
            avg_delta = mean(avg_deltas)
            self._offset = avg_delta
            self._client.log("Synchronized time, delta to local time =", avg_delta,
                             "simtime=", "{:5.1f}".format(self.simulation_time * 1000.0))
