...
EntityCollector::EntityIterator::EntityIterator(
    iter_t pos, iter_t end, Component::bitmask_t target_mask)
    : _pos(pos), _end(end), _target_mask(target_mask) {
  inc_until_valid();
}
...
void EntityCollector::print_status() {
  ECS_OUTPUT_DEBUG("  Collector with mask "
                   << std::hex << _component_mask << std::dec << " and "
                   << _matching_entities.size() << " entities:");
