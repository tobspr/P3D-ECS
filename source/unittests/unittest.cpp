...
void measure_time(const string& desc, function<void()> method, size_t num_iterations) 
{
    string iteration_desc = std::to_string(num_iterations);
    if (num_iterations > 1000000) {
        iteration_desc = std::to_string((size_t)((float)num_iterations / 1000000.0)) + " mio.";
    }

    TC_STATUS("Start time measure for operation '" << desc << "' with " << iteration_desc << " iterations.");
    ...
    write_tc_log("Measure: " + desc + " (" + std::to_string(num_iterations) + ") took " + std::to_string(ms) + " ms.\n");
}
