...
void measure_time(const string& desc, function<void()> method, size_t num_iterations = 1);

template < typename T >
void testsuite_expect(T value, T expected, const string& comparison, const char* file, size_t line)
{
	if (value != expected) {
        stringstream s;
        s << "\n\n========================================================================" << endl;
...
void testsuite_expect(A value, B expected, const string& comparison, const char* file, size_t line)
{
    // Forced cast from B to A to avoid error when choosing two different types (e.g. int and size_t)
    return testsuite_expect(value, (A)expected, comparison, file, line);
}


class EntityManager;
void general_testsuite(const string& name, function<void(EntityManager*)> inner);
