...
struct TransformComponent : public Component {
  ...
  inline TransformComponent(Entity *entity, const LVecBase3f &pos,
                            const LVecBase3f &scale, const LVecBase3f &hpr)
      : Component(entity), _pos(pos), _scale(scale), _hpr(hpr), _dirty(false),
        _parent(nullptr){};
  inline TransformComponent(Entity *entity)
      : Component(entity), _dirty(false), _parent(nullptr){};
  ...
  inline void set_mat(const LMatrix4f &mat) {
    _mat = mat;
    _dirty =
        true; /* todo: reconstruct pos and so on? or maybe set a flag like
                 _custom_mat */
  };
...
protected:
  inline void unregister_child(Entity *entity) {
    vector_erase_fast(_children, entity);
  };
  inline void register_child(
      Entity *entity) { /* TODO: assert(!_children.contains(entity)) */
...
struct PhysicsComponent : public Component {
  ...
  inline PhysicsComponent(Entity *entity, float mass,
                          const LVecBase2f &velocity)
      : Component(entity), velocity(velocity), mass(mass){};

  inline PhysicsComponent(Entity *entity) : Component(entity){};
};
